...
const Detection = () => {
  ...
  // Handle tab changes and auto-load history
  const handleTabChange = (value) => {
    setActiveTab(value);
    if (value === "history") {
      console.log("🔄 Switching to history tab, loading history...");
      loadHistory();
    }
  };
  ...
  const handleDetect = async () => {
    ...
    try {
      ...
      // Update history immediately with the new detection
      const newHistoryItem = {
        id: detectionResult.id,
        image: detectionResult.image_url,
  ...
  const loadHistory = async () => {
    try {
      ...
      const historyData = response.data.map(item => ({
        id: item.id, // Unique database ID
        image: item.image_url, // Base64 data URL from backend
  ...
  const handleDeleteHistory = async (detectionId = null) => {
    try {
      ...
      if (detectionId) {
        // Delete single item
        setHistory(prevHistory => prevHistory.filter(item => item.id !== detectionId));
        toast({
          title: "Deleted",
                        ...
                        <CardContent className="p-0">
                          <div className="aspect-square bg-gray-100 rounded-t-lg flex items-center justify-center relative">
                            <img
                              ...
                              onError={(e) => {
                                console.error(`Failed to load image for detection ${item.id}`);
                                e.target.src = 'https://via.placeholder.com/300x300?text=Image+Not+Found';
                                e.target.alt = 'Image failed to load';
                              }}
                              onLoad={() => console.log(`✅ Image loaded for detection ${item.id}`)}
                            />
                          ...
                          <div className="p-4 space-y-2">
                            <p className="text-sm text-muted-foreground">
                              <span className="font-medium">Detected:</span> {item.result}
                            </p>
                            <p className="text-sm text-muted-foreground">
