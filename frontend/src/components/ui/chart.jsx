...
const ChartTooltipContent = React.forwardRef(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const value = label || itemConfig?.label || item.name;
          ...
          {payload.map((item, index) => {
            ...
            return (
              <div key={item.dataKey} className={cn("flex w-full flex-wrap items-stretch gap-2", indicator === "dot" && "items-center")}>
                {!hideIndicator && <div style={{ backgroundColor: indicatorColor }} className="h-2.5 w-2.5 shrink-0 rounded-[2px]" />}
      ...
      {payload.map((item) => {
        ...
        return (
          <div key={item.value} className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground")}>
            {itemConfig?.icon && !hideIcon ? <itemConfig.icon /> : <div className="h-2 w-2 shrink-0 rounded-[2px]" style={{ backgroundColor: itemConfig?.color }} />}
            {itemConfig?.label}
          </div>
        );
      })}
